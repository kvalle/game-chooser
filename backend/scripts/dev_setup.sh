...
ansible-vault decrypt --output="app/config.py" "secretconfig/dev-config.py"
ansible-vault decrypt --output="firebase-serviceAccountKey.json" "secretconfig/dev-firebase-serviceAccountKey.json"

venv_bin_path=$(realpath --relative-to=${staring_dir} $(cd .venv/bin && pwd))
server_path=$(realpath --relative-to=${staring_dir} "$(pwd)/scripts/dev_server.py")
echo "All done!"
